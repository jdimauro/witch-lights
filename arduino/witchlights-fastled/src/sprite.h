...
// For testing use only. In production, set this equal to 1. Use this to
// exaggerate the acceleration effects. 10-20 is good for testing.
...
// The lower the value, the longer the trail generated, but also the more
// FastLED functions get called per update per sprite.
#define FAERIE_MIN_TRAIL_LENGTH 80
#define FAERIE_MAX_TRAIL_LENGTH 33

// currently set this to be consistent for animation design
// This range is dead center in my first "no idle" zone, so any problems
// with the no-idle functions will show up quickly
#define FAERIE_MIN_LONG_TRAVEL 50
...
const bool randomInflection = false;  // Randomly makes faerie sprite dance back
                                      // and forth, instead of mainly going
                                      // "forwards".
...
// What I'm going for here: I want to put these in shadowed places, where our
// primate brain will be looking for glowing eyes in the night. I want them to
// awaken, and blink, and maybe shift a little, and blink.
class LurkerSprite : public Sprite {
 private:
  ...
  int blinkCount;
  int blinkFrequency;
  int eyeWidth;                 // spacing between eyes, minimum is 1, set this higher the
                                // further away you are locating the sprite from the viewer
  int blinkDirection;           // if in blink mode, are we closing or opening? -1 for
                                // closing, +1 for opening, 0 for staring
  ...
  // We're defining a set of either 2, 4, or 6 pixels, depending on how far
  // apart you want the eyes to be
  ...
  int SetBlinkTiming() {
    // debug(5);
    return random(LURKER_BLINK_SHUT_MIN_TIMING,
                  LURKER_BLINK_SHUT_MAX_TIMING +
                      1);  // ms; testing to see what looks good, these are rough guesses
  }
 ...
 public:
  LurkerSprite(int spawnPixel, int eyew) : Sprite() {
    ...
    this->blinkCount = -1;                       // When first opening the eyes, it "counts" as a
                                                 // blink, and setting to -1 means we don't count it
                                                 // against lifespan
  ...
  bool allowCreation() {
    return (currentPixel >= LURKER_MIN_PIXEL_1 && currentPixel <= LURKER_MAX_PIXEL_1) ||
           (currentPixel >= LURKER_MIN_PIXEL_2 && currentPixel <= LURKER_MAX_PIXEL_2) ||
           (currentPixel >= LURKER_MIN_PIXEL_3 && currentPixel <= LURKER_MAX_PIXEL_3);
  }
...
class LoopTestSprite : public Sprite {
 private:
  ...
  // pattern is one black pixel plus remaining pixels in order of increasing
  // brightness with brightest pixel doubled.
  ...
  int GetNewScanCountTotal() { return random(FAERIE_MIN_IDLE, FAERIE_MAX_IDLE + 1); }
 ...
 public:
  LoopTestSprite() : Sprite() {
    ...
    for (int i = 0; i < afc_l_pulsar_a_ANIMATION_FRAME_WIDTH * afc_l_pulsar_a_ANIMATION_FRAMES;
         i++) {
      af_l_pulsar_a[i] =
          afc_l_pulsar_a[i] > ' ' ? colorSets[colorPalette][afc_l_pulsar_a[i] - '0'] : CRGB::Black;
    }
  }
...
class FaerieSprite : public Sprite {
 private:
  ...
  int idleCount;
  int idleCountTotal;
  float accelerationFactor;  // make these semi-randomly set parameters on
                             // create; range from 0.5 to 2?
  float brakeFactor;
  int dimFactor;
  ...
  // use updateInterval and map() to make tails longer when going faster (reduce
  // the fade factor) and shorter when going slower (increase fade factor)
  int SetDimFactor(int interval) {
    return map(interval, 0, SPRITE_STARTING_DELAY_INTERVAL_IN_MS, trailLength, 192);
  }

  int SetTrailLength() { return random(FAERIE_MAX_TRAIL_LENGTH, FAERIE_MIN_TRAIL_LENGTH) + 1; }
  ...
  void SetNextInflection() {
    ...
    // TODO when direction (1 or -1) is added as a sprite param on create, use
    // that to make the sprite "advance" in the sprite's general direction(?)
  ...
  void SetNextWaitTravelTarget() {
    // TODO use collision detection when setting next inflection, so that
    // faeries are never traveling TO the same target pixel (or within a range
    // of that pixel, like +- 3)

    lastInflection = nextInflection;
    int travelDistance =
        (random(FAERIE_FLIT_MIN_DISTANCE, FAERIE_FLIT_MAX_DISTANCE) + 1) * TravelDirectionSwitch();
    nextInflection += travelDistance;
  }
  ...
  int GetNewidleCountTotal() { return random(FAERIE_MIN_IDLE, FAERIE_MAX_IDLE) + 1; }

  void FadeToColor() {
    // https://gist.github.com/kriegsman/d0a5ed3c8f38c64adcb4837dafb6e690
    // https://gist.github.com/kriegsman/1f7ccbbfa492a73c015e
    // want to have the sprite's two bright pixels fade from one color set to
    // another
  }

  // add int direction and make tailpixel += direction, and when direction is
  // "backwards", offset the dimtrail by 3 pixels to prevent dimming the sprite
  ...
  void FadeOutTrail(int tailPixel, int dimFactor, int direction) {
    ...
    // Recursively run DimTrail() at tailPixel+1, so trail fades from the dim
    // end
    DimTrail(tailPixel, dimFactor, direction);
  ...
  int DistanceFromDestination() { return abs(currentPixel - nextInflection); }

  int SetMaxSpeed() { return random(FAERIE_MIN_SPEED, FAERIE_MAX_SPEED) + 1; }

  int SetMaxBrake() { return random(FAERIE_MIN_BRAKE, FAERIE_MAX_BRAKE) + 1; }

  float SetBrakePercentage() {
    // TODO: map distance of move to brake percentage, brakefactor,
    // accelerationfactor
    if (!isWaiting) {
      return .12;
    } else {
  ...
  int SetWaitCount() { return random(FAERIE_MIN_WAIT, FAERIE_MAX_WAIT) + 1; }

  int SetWaitInterval() {
    return random(FAERIE_FLIT_MIN_START_INTERVAL, FAERIE_FLIT_MAX_START_INTERVAL) + 1;
  }

  int EffectiveFrame(int frame) { return frame % idleFrameCount; }

  int IdlePixelA(int frame) { return 8 - abs(EffectiveFrame(frame) - 8); }
  ...
  int IdlePixelC(int frame) { return abs(EffectiveFrame(frame) - 8); }
  ...
  int AccelerateTravel() {
    ...
    // When waiting (short moves) we use a different acceleration and braking
    // factor than we do with longer moves. You need exaggerated acceleration
    // and braking over shorter distances.
    SetBrakeFactor(isWaiting);
    SetAccelerationFactor(isWaiting);

    // Accelerate or brake by returning positive or negative values to subtract
    // from updateInterval
    if (!isBraking) {
  ...
  void JitterLocation() {
          ...
          if (EffectiveFrame(idlingFrame) == 0 && random(0,3) == 0 && !moveIt) {
  // pixelA == 8
                  currentPixel --;
                  updateInterval = 18; // updateInterval * 2;
          } else if (EffectiveFrame(idlingFrame) == 8 && random(0,3) == 0 &&
  !moveIt) { // pixelC == 8
                  currentPixel ++;
                  updateInterval = 18; // updateInterval * 2;
          }
 ...
 public:
  FaerieSprite() : Sprite() {
    ...
    // To be set semi-randomly for all sprites, making them move just a bit
    // differently so that they never mirror each other
    ...
    /*
    for (int i = 0; i < afc_l_mother_ANIMATION_FRAME_WIDTH *
    afc_l_mother_ANIMATION_FRAMES; i++) {
            af_l_mother[i] = afc_l_mother[i] > ' ' ?
    colorSets[colorPalette][afc_l_mother[i] - '0'] : CRGB::Black;
    }
    */
  }
  ...
  int ReportLocation() {
    ...
    // TODO: if drawDirection = -1, return the pixel that it's being "drawn" on;
    // subtract currentPixel from NUM_LEDS

    return currentPixel;
  }
};

// Test intro and outro fragments together until I like them, then split into
// two halves and adjust code to play one, a loop, and then the other.
class FragmentTestSprite : public Sprite {
 ...
 public:
  FragmentTestSprite() : Sprite() {
    ...
    // Read all frames of animation at once into the af_f_slow_stop[i] CRGB
    // struct, which is then used to write to the FastLED leds buffer
    // If we want to automatically create trails, we need to have made the
    // correct changes to the char afc_f_slow_stop's contents to create
    // the fading trail.
    for (int i = 0; i < afc_l_pulsar_a_ANIMATION_FRAME_WIDTH * afc_l_pulsar_a_ANIMATION_FRAMES;
         i++) {
      af_l_pulsar_a[i] =
